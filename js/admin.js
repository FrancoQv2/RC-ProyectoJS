...
function cleanForm() {
    ...
    nature1.classList = "form-control";
        ...
        if (name2.value == "" || origin2.value == "" || power2.value == "" || strenght2.value == "" || image2.value == "") {
            ...
            for (i in _fighters) {
                if (_fighters[i].id == id2.value) {
                    let fighter = new Fighter(id2.value,nature2.value, name2.value, origin2.value, power2.value, strenght2.value, image2.value);
                    _fighters[i] = fighter;
                    localStorage.setItem('_fighters', JSON.stringify(_fighters));
          ...
          <span aria-hidden="true" class="w-120 bigger-close">&times;</span>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group col-sm-12 col-md-6">
                            ...
                            <div class="form-group col-sm-12 col-md-6">
                                <label class="labels-modal" for="strenght">Nivel de rudeza</label>
                            ...
                            <div class="form-group col-sm-12 col-md-6">
                                <label class="labels-modal" for="image">Link Imagen</label>
                                <input placeholder="${peleadores.image}" type="text" class="form-control" id="image2" onchange="controls2()">
                            </div>
                            <button type="submit" class="btn btn-danger w-20 mx-3 mt-2">Modificar personaje</button>
                        </form>
                    </div>
                </div>
            </div>
...
function showTable() {
    ...
    _fighters.map((item, i) => {
        table.innerHTML += 
        `<tr>
            <td class="skew-font"> ${item.name}</td>
            <td class="text-wrap shortened ">${item.image}</td>
            <td ">${item.nature}</td>
            <td " >${item.power}</td>
            <td ">${item.strenght}</td>
            <td ">${item.origin}</td>
            <td class="btn btn-outline-danger" title="Borrar" onclick="deleteFighter(${i})"><i class="fas fa-trash-alt up-icon"></i></td>
            <td class="btn btn-outline-danger" title="Modificar" data-toggle="modal" data-target="#login-modal" onclick="updateFighter(${i})">
            <i class="fa fa-pencil-alt my-auto"></i></td>
        </tr>`
    })
}
